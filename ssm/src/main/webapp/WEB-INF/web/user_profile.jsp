				...
				<form id="user-profile">
					...
					<div class="rs_content_sex">
						<span class="same">性别： </span> <input type="radio" name="gender"
							id="gender_male" value="1"
							<c:if test="${user.gender == 1}">checked</c:if>>男 <input
							type="radio" name="gender" id="gender_female" value="0"
							<c:if test="${user.gender == 0}">checked</c:if>>女
						<!-- 以下是原来的使用图片，不使用 -->
					...
					<div class="rs_content_tel">
						<span class="same">绑定电话：</span> <input name="phone" id="phone"
							type="text" value="${user.phone}" />
					</div>
					<!--绑定邮箱-->
					<div class="rs_content_mail">
						<span class="same">绑定邮箱：</span> <input name="email" id="email"
							class="ed_email" value="" style="display: none;" /> <span
							class="rs_mail">${user.email}</span> <span
							class="same_click change_mail">更改邮箱</span>
					</div>
					<div class="buttons-block" id="editProfile">
						<a href="#" class="button-blue">保存更改</a>
					</div>
				</form>
			</div>
		</div>
